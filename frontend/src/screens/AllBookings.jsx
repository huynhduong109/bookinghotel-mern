                ...
                <Fragment>
                    {isLoading ? <Loader /> : (
                        <div className="w-[80%] sm:w-[60%] md:w-[70%] mx-auto mt-3">
                                        ...
                                        {(rowsPerPage > 2 ? allBookings?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage) : allBookings)?.map((booking) => (
                                            <TableRow key={booking._id} style={{ height: 72.8 }}>
                                                ...
                                                <TableCell align="center">
                                                    {booking.status}
                                                    <Tooltip title={booking.status === "Đã hoàn thành" ? "" : "Edit"} placement="right" className="!ml-2">
                                                        <IconButton onClick={() => {
                                                            setOpen(!open);
                                                            setBookingRef(booking);
                                                            setStatus(booking.status);
                                                        }}
                                                            disabled={booking.status === "Đã hoàn thành" || booking.status === "Canceled" ? true : false}
                            ...
                            <Dialog open={open}>
                                <div className="p-5">
                                    <DialogTitle className="text-center">Thay đổi trạng thái đặt phòng</DialogTitle>
                                    ...
                                    <DialogActions className="mt-4">
                                        <button onClick={() => setOpen(!open)} className="bg-red-400 hover:bg-red-500 py-2 rounded-lg w-24 text-center text-neutral-50  transition duration-200 font-semibold">Trở về</button>
                                        <button
