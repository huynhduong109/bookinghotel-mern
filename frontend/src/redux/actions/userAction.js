...
import { setAllBookings } from '../slices/hotelSlice';
...
export const cancelBooking = (bookingId) => async(dispatch) => {
    try {
        const { data } = await axios.put(`/api/v1/me/booking/${bookingId}/cancel`, { headers: { "Content-Type": "application/json" } });
        dispatch(setAllBookings(data.bookings));
        dispatch(setSuccess("Hủy đặt phòng thành công"));
    } catch (err) {
        dispatch(setError(err.response.data.message));
    }
};
